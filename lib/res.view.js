...
module.exports = function (kites) {
     ...
     * @param {Function} next
     */
    return function _addResViewMethod(req, res, next) {
        ...
        res.view = function resView( /* specifiedPath, locals, optionalCb */ ) {
            ...
            if (_.isUndefined(res.locals.view)) {
                res.locals.view = {
                    ...
                    pathFromApp: path.join(path.relative(kites.express.options.appPath, kites.express.options.paths.views), relPathToView),
                    ext: kites.express.options.views.extension
                };
            }
            ...
            if (process.env.NODE_ENV !== 'production') {
                res.locals.__dirname =
                    res.locals.__dirname ||
                    (absPathToView + '.' + kites.express.options.views.extension);
            }
            ...
            kites.logger.debug('Serving view at rel path: ', relPathToView);
            kites.logger.debug('View root: ', kites.express.options.paths.views);
