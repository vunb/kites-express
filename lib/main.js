...
class KitesExpress {
    ...
    configureViewEngine(app, opts) {
        var configView = this.kites.emit('expressConfigure:view', app);
        if(configView) {
        ...
        try {
            ...
            app.engine(opts.name, engine);
            app.set('view engine', opts.name);
            app.set('views', opts.path)
        } catch (err) {
            this.kites.logger.error('Configure view engine erro: ', err);
        }
    }
