...
module.exports = function (kites) {
    return function _mixinReq(req, res, next) {
         ...
         *
         * */
        res.serverError = function sendServerError(err) {
            if (_.isString(err)) {
                err = {
                    message: err
                }
         ...
         * ```
         */
        res.forbidden = function sendErrorForbidden(err) {
            ...
            if (req.wantsJSON) {
                ...
                // check data error
                if (_.isError(err) && !_.isFunction(err.toJSON)) {
                    err = err.stack;
                    return res.send(err);
                }
                // send json error
